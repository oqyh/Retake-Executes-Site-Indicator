...
{
	name = "Executes Site Indicator",
	author = "Gold_KingZ",
	description = "Show Executes Site Chat , Alert , Sound",
	version = "1.0.0",
...
{
	LoadTranslations( "Executes_Site_Indicator.phrases" );
	
	g_enable_chat = CreateConVar("sm_executes_chat_site"		  	 , "1", "Enable chat  site indicator || 1= Yes || 0= No", _, true, 0.0, true, 1.0);
	
	g_enable_hint_before = CreateConVar("sm_executes_hint_before"		  	 , "1", "Enable hint site indicator BEFORE PLANT || 1= Yes || 0= No", _, true, 0.0, true, 1.0);
	g_THud2 = CreateConVar("sm_executes_hud_before", "5", "How long hint bomb site HUD should be displayed BEFORE PLANT", _, true, 1.0);
	
	g_enable_hint_after = CreateConVar("sm_executes_hint_after"		  	 , "1", "Enable hint  site indicator AFTER PLANT || 1= Yes || 0= No", _, true, 0.0, true, 1.0);
	g_THud = CreateConVar("sm_executes_hud_after", "5", "How long hint bomb site HUD should be displayed AFTER PLANT", _, true, 1.0);

	g_enable_sound = CreateConVar("sm_executes_sound_site"		  	 , "1", "Enable sound  site indicator || 1= Yes || 0= No", _, true, 0.0, true, 1.0);
	PickUpSoundName = CreateConVar("sm_executes_sound_sitea", "gold_kingz/sitea/ExecutesOnAsite.mp3", "Path Sound For Site A");
	PickUpSoundName2 = CreateConVar("sm_executes_sound_siteb", "gold_kingz/siteb/ExecutesOnBsite.mp3", "Path Sound For Site B");
	...
	AutoExecConfig(true, "Executes_Site_Indicator");
}
...
void ShowInfo()
{
	Bombsite site = Executes_GetCurrrentBombsite();
...
public Action Timer_DisplayHUD2(Handle timer, any data)
{
	...
	{
	PrintCenterTextAll("<font face='Arial' size='20'> %t </font>\n\t<span class='fontSize-xxxl'><font face='Arial' color='#00FF00'><b>%t</b></font></font>", "ExecutesOn", "BombPlantA");
	}else if (site == BombsiteB && g_benable_hint_before)
	{
	PrintCenterTextAll("<font face='Arial' size='20'> %t </font>\n\t<span class='fontSize-xxxl'><font face='Arial' color='#00FF00'><b>%t</b></font></font>", "ExecutesOn", "BombPlantB");
	}
	return Plugin_Continue;
}
...
stock bool Executes_Live() {
    return Executes_Enabled() && !InWarmup();
}
